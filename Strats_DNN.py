...
def getPrediction(X_temp_train, Y_temp_train, X_temp_test, batch_size, nb_epoch):
    # First deal with nan falue
    X_temp_train = np.nan_to_num(X_temp_train)
    X_temp_test = np.nan_to_num(X_temp_test)
    # Start
    num_fet = np.shape(X_temp_train)[1]
    node_hide =np.int((num_fet + np.shape(Y_temp_train)[0]) / 2)
    ...
    classifier.add(Dense(output_dim = node_hide, init = 'uniform', activation = 'relu', input_dim = num_fet)) 
    # Adding the second hidden layer
    classifier.add(Dense(output_dim = node_hide, init = 'uniform', activation = 'relu'))
...
# Function to test if the prediction is accurate
def getTestProb(stock_prob, Y, threshold):
    test_results = dict()
    test_prob = (stock_prob > threshold)
    num_days = len(stock_prob)
    Y_test = Y.tail(num_days)
    for d in xrange(num_days):
        cm = confusion_matrix(Y_test.iloc[d, :].values, test_prob.iloc[d, :].values)
        test_results[d] = cm
    return test_results
...
def getDailyReturn(pos, daily_stk_ret):
    ...
    for d in xrange(len(pos)):
        temp_ret = pos.iloc[d, :].dot(daily_stk_ret.iloc[d + 2, :])
...
#    fet_days = list([1, 2, 5, 9, 12, 26, 50, 60, 80, 100, 150, 200, 250]) 
    fet_days = list([1, 2, 5, 12, 26, 60, 80, 100, 120])
#    fet_days = list([1, 2, 5])
    ...
    for d in xrange(len(X_dict) - 2):
        ...
        pred = getPrediction(X_temp_train, Y_temp_train, X_temp_test, batch_size, nb_epoch)
        pred = pd.DataFrame(pred)
        pred = pred.fillna(value = np.median(pred))
        if d == 0:
            stock_prob = pd.DataFrame(pred)
        else:
            stock_prob = pd.concat([stock_prob, pd.DataFrame(pd.DataFrame(pred))], axis = 1)
        print "Day%d completed\n" %(d)
    ...
    # Print the back-test results
    Sharpe, cum_ret = getResults(daily_return, rf)
    
    # Test the prediction results
    threshold = 0.5
    pre_results = getTestProb(stock_prob, Y, threshold)
